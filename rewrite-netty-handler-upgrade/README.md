...
3. 带 authored-source 前置条件的官方 `org.openrewrite.java.AddLiteralMethodArgument` constructor
   支持：迁移 `RuleBasedIpFilter(IpFilterRule...)`；
4. 同一个官方 recipe 的 method-invocation 支持：迁移 `SslHandler.isEncrypted(ByteBuf)`；
...
| 官方能力 | 审计结论 | 本模块处理 |
| --- | --- | --- |
| [`rewrite-netty:0.10.3`](https://github.com/openrewrite/rewrite-netty/tree/9546824604ff662eb73bd4cabdd9a9d54bc0ae63) catalog | 固定 JAR 的 `recipes.csv` 和 YAML 只有 3.2→4.1、4.1→4.2、EventLoopGroup 等跨线迁移，没有 4.1 patch-level Handler/TLS 配方 | 作为 test-scope 官方审计依赖；runtime 测试激活并检查真实 recipe tree |
| [`org.openrewrite.netty.UpgradeNetty_4_1_to_4_2`](https://github.com/openrewrite/rewrite-netty/blob/9546824604ff662eb73bd4cabdd9a9d54bc0ae63/src/main/resources/META-INF/rewrite/netty-4_1_to_4_2.yml) | runtime tree 包含宽泛 `UpgradeDependencyVersion(io.netty:*, 4.2.x)`、两个 `ChangeDependency`、`ChangeType`、`ChangePackage` 和 EventLoopGroup 迁移；与精确 4.1.136 目标相反 | 明确断言推荐 tree 不包含这些节点，避免改成 4.2 分支迁移 |
...
| [`AddLiteralMethodArgument`](https://github.com/openrewrite/rewrite/blob/b3008cc4a1f0c43f562da16e5933a2a56d9bc568/rewrite-java/src/main/java/org/openrewrite/java/AddLiteralMethodArgument.java) method 支持 | 可精确匹配一参数 `SslHandler.isEncrypted(ByteBuf)` 并在索引 1 加布尔字面量 | **组合复用**，配置 `literal:false`、`primitiveType:boolean` |
| 同一官方 recipe 的 [`visitNewClass` / constructor 支持](https://github.com/openrewrite/rewrite/blob/b3008cc4a1f0c43f562da16e5933a2a56d9bc568/rewrite-java/src/main/java/org/openrewrite/java/AddLiteralMethodArgument.java) | 固定源码原生访问 `J.NewClass`、匹配 `<constructor>` 并同步更新 method type；官方固定测试 `addToConstructorArgument` 证明该能力。审计同时复现 8.87.5 在索引 0 插入时输出 `( true,rules)` 的 prefix 缺陷 | **替换原本地语义 AST visitor**；官方 recipe 在索引 0 加 `true`，随后由仅匹配已迁移、带类型归属构造器的本地格式适配器修复两个受影响 prefix，不重做参数或 method type 迁移。测试覆盖零参数 varargs、数组、expanded varargs、求值顺序、注释/换行、类型归属与幂等 |
| OpenRewrite Netty 4.1 patch API recipe | 官方 catalog 当前没有 4.1.x patch 内的 Handler/TLS/SNI/timeout 迁移 | 本模块补足两项等价 AUTO 和精确 MARK |

组合测试直接激活推荐 YAML，证明两个官方 `AddLiteralMethodArgument` 实例同时生效，
并证明两者都不会修改 `target/generated-sources`。另一个 runtime-tree 测试实际激活固定
`org.openrewrite.netty.UpgradeNetty_4_1_to_4_2`，核对其宽配方子树，同时递归断言推荐 tree 中没有
`UpgradeDependencyVersion`、`ChangeDependency` 或 EventLoopGroup 4.2 迁移。
测试结构参考官方固定提交的
...
原有 rule 表达式仍按原顺序、各求值一次。语义转换由官方
`AddLiteralMethodArgument` 的 `<constructor>` 支持完成，本模块不再维护重复语义 AST visitor。
由于固定 Core 8.87.5 对索引 0 的新 literal 使用 `Space.SINGLE_SPACE`、同时保留旧首参数的空 prefix，
会打印为 `( true,rules)`，模块只在带类型归属的
`RuleBasedIpFilter(boolean, IpFilterRule[])` 上运行格式适配器，将其稳定为 `(true, rules)`；
原有换行和注释 prefix 保持不变。
自动显式化 `true` 不代表策略已经安全；同一节点仍会 MARK，要求确认“无规则命中时允许”是否符合业务安全策略。
...
| 固定来源 | 抽取行为 | 覆盖结果 |
| --- | --- | --- |
| [Apache Flume `AvroSource` @ `579b77c`](https://github.com/apache/logging-flume-rpc/blob/579b77c28000e19c3ee10aca1677202dfa951f72/flume-rpc-avro/src/main/java/org/apache/flume/rpc/avro/source/AvroSource.java) | `new RuleBasedIpFilter(rules.toArray(...))` | 官方 constructor AUTO 加 `true`，保持数组求值 |
...
- Maven 唯一/root/profile imported BOM、歧义 owner、Gradle `platform`/`enforcedPlatform` 和
  versionless handler；
- 两个官方 Core recipe 与严格自研 dependency/risk recipe 的组合、阶段顺序和两周期幂等；
- 固定 `rewrite-netty:0.10.3` runtime recipe tree 与宽泛 4.2 子树的排除断言；
