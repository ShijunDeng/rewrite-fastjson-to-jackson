...
public final class UpgradeSelectedTwelveMonkeysJpegDependency extends Recipe {
    private static final String PREFIX = TwelveMonkeysJpegSupport.GROUP + ":" + TwelveMonkeysJpegSupport.ARTIFACT + ":";
    private static final Pattern PROPERTY_REFERENCE = Pattern.compile("\\$\\{([^}]+)}");

    @Override
    public String getDisplayName() {
        return "Upgrade workbook-selected TwelveMonkeys ImageIO JPEG dependency to 3.12.0";
    }

    @Override
    public String getDescription() {
        return "Upgrades only exact com.twelvemonkeys.imageio:imageio-jpeg version 3.9.3 in owned standard Maven or Gradle dependency declarations.";
    }
    ...
    private static G.CompilationUnit migrateGroovy(G.CompilationUnit source, ExecutionContext ctx) {
        return (G.CompilationUnit) new GroovyIsoVisitor<ExecutionContext>() {
            ...
            public J.MethodInvocation visitMethodInvocation(J.MethodInvocation method, ExecutionContext executionContext) {
                boolean dependency = TwelveMonkeysJpegSupport.isGradleDependencyInvocation(getCursor(), method);
                J.MethodInvocation visited = super.visitMethodInvocation(method, executionContext);
                if (!dependency) return visited;
                if (coordinate(TwelveMonkeysJpegSupport.mapValue(visited, "group"), TwelveMonkeysJpegSupport.mapValue(visited, "name")) &&
            ...
            public J.Literal visitLiteral(J.Literal literal, ExecutionContext executionContext) {
                boolean dependency = TwelveMonkeysJpegSupport.isDirectDependencyLiteral(getCursor());
    ...
    private static Xml.Document migratePom(Xml.Document document, ExecutionContext ctx) {
        ...
        Set<PropertyOwner> safe = targetReferences.keySet().stream()
                .filter(owner -> TwelveMonkeysJpegSupport.SOURCE.equals(values.get(owner)))
        ...
        return (Xml.Document) new XmlIsoVisitor<ExecutionContext>() {
            ...
            public Xml.Tag visitTag(Xml.Tag tag, ExecutionContext executionContext) {
                ...
                if (TwelveMonkeysJpegSupport.isJpegDependency(getCursor(), visited) && TwelveMonkeysJpegSupport.standardJar(visited) &&
                    visited.getChildValue("version").map(String::trim).filter(TwelveMonkeysJpegSupport.SOURCE::equals).isPresent()) {
    ...
    private static boolean targetVersionReference(Cursor cursor, String text) {
        ...
        return dependencyCursor.getValue() instanceof Xml.Tag dependency &&
               TwelveMonkeysJpegSupport.isJpegDependency(dependencyCursor, dependency) && TwelveMonkeysJpegSupport.standardJar(dependency);
    }
    ...
    private static boolean coordinate(String group, String artifact) {
        return TwelveMonkeysJpegSupport.GROUP.equals(group) && TwelveMonkeysJpegSupport.ARTIFACT.equals(artifact);
    }

    private static G.MapLiteral upgradeMap(G.MapLiteral map) {
        if (!coordinate(TwelveMonkeysJpegSupport.mapValue(map, "group"), TwelveMonkeysJpegSupport.mapValue(map, "name")) ||
            !TwelveMonkeysJpegSupport.SOURCE.equals(TwelveMonkeysJpegSupport.mapValue(map, "version")) || TwelveMonkeysJpegSupport.hasVariant(map)) return map;
    ...
    private static J.Literal upgradeCoordinate(J.Literal literal) {
        if (!(literal.getValue() instanceof String value) || !(PREFIX + TwelveMonkeysJpegSupport.SOURCE).equals(value)) return literal;
        return TwelveMonkeysJpegSupport.replaceLiteral(literal, PREFIX + TwelveMonkeysJpegSupport.TARGET);
    }
