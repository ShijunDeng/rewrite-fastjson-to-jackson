...
/** Upgrades only source versions explicitly selected by the migration workbook. */
public final class UpgradeSelectedFeignCoreDependency extends Recipe {
    static final String GROUP = "io.github.openfeign";
    static final String ARTIFACT = "feign-core";
    static final Set<String> SOURCE_VERSIONS = Set.of("10.4.0", "11.1", "11.9", "12", "12.1", "12.4");
    static final String TARGET = "13.6";
    ...
    public String getDisplayName() {
        return "Upgrade workbook-selected Feign Core dependencies to 13.6";
    }

    @Override
    public String getDescription() {
        return "Upgrade only exact io.github.openfeign:feign-core source versions listed by the workbook in owned Maven " +
               "or Gradle declarations, without guessing ranges or external version owners.";
    }
                    ...
                    return new GroovyIsoVisitor<ExecutionContext>() {
                        ...
                        public J.MethodInvocation visitMethodInvocation(J.MethodInvocation method,
                            ...
                            if (GROUP.equals(invocationMapValue(m, "group")) &&
                                ...
                                SOURCE_VERSIONS.contains(invocationMapValue(m, "version")) && !hasVariant(m)) {
    ...
    private static Xml.Document migratePom(Xml.Document document, ExecutionContext ctx) {
        ...
        Map<PropertyOwner, Integer> feignReferences = new HashMap<>();
        new XmlIsoVisitor<ExecutionContext>() {
            @Override
            public Xml.CharData visitCharData(Xml.CharData charData, ExecutionContext executionContext) {
                ...
                collectReferences(visited.getText(), getCursor(), definitions, allReferences,
                        feignVersionReference(getCursor(), visited.getText()) ? feignReferences : null);
                return visited;
            }
        ...
        Set<PropertyOwner> safeProperties = feignReferences.keySet().stream()
                ...
                .filter(owner -> feignReferences.getOrDefault(owner, 0) > 0)
                .filter(owner -> allReferences.getOrDefault(owner, 0)
                        .equals(feignReferences.getOrDefault(owner, 0)))
        ...
        return (Xml.Document) new XmlIsoVisitor<ExecutionContext>() {
            ...
            public Xml.Tag visitTag(Xml.Tag tag, ExecutionContext executionContext) {
                ...
                if (isFeignCoreDependency(getCursor(), t) && t.getChildValue("version").map(String::trim)
    ...
    static boolean isFeignCoreDependency(Cursor cursor, Xml.Tag tag) {
        ...
        for (Cursor current = cursor.getParent(); current != null; current = current.getParent()) {
            if (current.getValue() instanceof J.MethodInvocation ancestor) {
                if (!foundRootDependencies) {
                    ...
                    // buildscript/subprojects/allprojects/custom DSL dependencies are not root project dependencies.
                    return false;
                }
            }
        }
    ...
    private static boolean feignVersionReference(Cursor cursor, String text) {
        ...
        return dependencyCursor.getValue() instanceof Xml.Tag dependency &&
               isFeignCoreDependency(dependencyCursor, dependency);
    }
    ...
    private static J.Literal replaceLiteral(J.Literal literal, String newValue) {
