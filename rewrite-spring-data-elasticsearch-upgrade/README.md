...
- 仅在构建文件能证明存在表格列出的六个版本时触发升级。只含 `4.2.5`、`4.3.x`、`4.4.13`、任意 `5.x` 或其他 `6.x` 的文件会保持不变；若同一文件同时声明一个表格版本和同坐标的其他旧版本，OpenRewrite 会把该文件中的旧声明一并对齐到 `6.0.5`，因此仍应审阅多 source set/profile 的 diff。
- 处理 Maven 直接依赖、版本属性、`dependencyManagement` 及活动 profile 中的显式版本。
- 处理 Gradle Groovy DSL 的直接字符串、map notation，以及有 Tooling API 模型的 Kotlin 直接字面量，并保留 `api`、`implementation`、`compileOnly`、`testImplementation` 等 configuration。为避免同文件中无关的相同数字造成误命中，间接版本变量保持不变，需在配方 diff 后人工更新。
...
- 用 OpenRewrite `SearchResult` 标记 RHLC/TransportClient、旧 template/configuration、ES 7 直连类型、删除方法、旧 mapping 注解/日期常量、repository `@Query`、Elasticsearch 源码中的 Spring cache 注解及 reactive 断点。
...
SearchResult 会以 `/*~~>*/` 出现在 dry-run diff 中，代表必须由项目负责人选择新 API 或补回归测试，而不是迁移已完成。
...
| BOM/platform 管理或未列版本 | 不补版本、不越界升级 | `leavesBomManagedVersionlessDependencyUntouched`、`leavesUnlistedVersionsUntouched` |
...
| Maven 属性、dependencyManagement、活动 profile | 自动处理本地显式版本 | 外部 parent/BOM 在定义处升级 |
...
| 仅含未列版本的文件、相似坐标和插件 | 保持不变 | 同文件混合版本需审阅；其余另建任务评估 |
...
当前 47 个 JUnit 测试覆盖：六个表格版本的 Maven 和 Gradle 升级、四类 Gradle configuration、Maven 属性/dependencyManagement/活动 profile、Gradle 字符串/map、变量安全 no-op、Kotlin DSL、scope/optional/exclusions 保留、四个真实仓库的构建与源码 before→after/marker、八类确定性 FQN 迁移、ES 7 client/query DSL、删除方法、旧注解/日期常量、repository/cache 语义与无关源码负例、BOM 管理无版本依赖、未列版本、相似坐标、插件不误改，以及公开 recipe discovery/validation。
