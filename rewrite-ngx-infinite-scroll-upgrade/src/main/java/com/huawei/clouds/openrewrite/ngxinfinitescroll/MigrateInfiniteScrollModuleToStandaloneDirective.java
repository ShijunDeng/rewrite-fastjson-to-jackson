...
/** Replaces the deprecated module only when all references have a deterministic standalone equivalent. */
public final class MigrateInfiniteScrollModuleToStandaloneDirective extends Recipe {
    private static final String MODULE = "InfiniteScrollModule";
    private static final String DIRECTIVE = "InfiniteScrollDirective";

    @Override
    public String getDisplayName() {
        return "Migrate InfiniteScrollModule to InfiniteScrollDirective";
    }

    @Override
    public String getDescription() {
        return "Replaces one import-resolved ngx-infinite-scroll module named import and its direct Angular " +
               "imports/exports array references when the file contains no other uses or existing directive import.";
    }
        ...
        return new JavaScriptIsoVisitor<ExecutionContext>() {
            ...
            public JS.Import visitImportDeclaration(JS.Import declaration, ExecutionContext ctx) {
                ...
                List<JS.ImportSpecifier> elements = ListUtils.map(named.getElements(), element -> {
                    if (!MODULE.equals(NgxInfiniteScrollSourceSupport.importedName(element))) return element;
            ...
            public JS.PropertyAssignment visitPropertyAssignment(JS.PropertyAssignment property, ExecutionContext ctx) {
                ...
                return visited.withInitializer(array.withInitializer(ListUtils.map(initializer, expression ->
                        expression instanceof J.Identifier identifier && migration.localName.equals(identifier.getSimpleName())
            ...
            private Migration findMigration(JS.CompilationUnit cu, ExecutionContext ctx) {
                ...
                new JavaScriptIsoVisitor<ExecutionContext>() {
                    ...
                    public JS.Import visitImportDeclaration(JS.Import declaration, ExecutionContext scanCtx) {
                        ...
                        if (!UpgradeSelectedNgxInfiniteScrollDependency.PACKAGE.equals(
                                NgxInfiniteScrollSourceSupport.moduleName(visited))) return visited;
                        JS.ImportClause clause = visited.getImportClause();
                        ...
                        for (JS.ImportSpecifier element : named.getElements()) {
                            ...
                            if (MODULE.equals(imported)) modules.add(new ImportCandidate(
                                    visited.getId(), NgxInfiniteScrollSourceSupport.localName(element)));
                        }
                        return visited;
                    }
                }.visit(cu, ctx);
                ...
                int expectedLocalOccurrences = 1 + metadataReferences;
                if (wordOccurrences(source, candidate.localName) != expectedLocalOccurrences) return null;
    ...
    private static boolean isAngularScopeProperty(JS.PropertyAssignment property, Cursor cursor) {
        String name = NgxInfiniteScrollSourceSupport.propertyName(property);
        ...
        while (ancestor != null && !(ancestor.getValue() instanceof J.NewClass)) {
            if (ancestor.getValue() instanceof JS.PropertyAssignment ||
                ancestor.getValue() instanceof J.Annotation ||
        ...
        return owner instanceof J.MethodInvocation invocation &&
               ...
               "TestBed".equals(NgxInfiniteScrollSourceSupport.expressionName(invocation.getSelect()));
    }
