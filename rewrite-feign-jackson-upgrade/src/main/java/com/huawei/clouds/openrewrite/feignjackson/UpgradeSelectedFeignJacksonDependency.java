...
/** Upgrade only the source versions selected by the migration workbook. */
public final class UpgradeSelectedFeignJacksonDependency extends Recipe {
    static final String GROUP = "io.github.openfeign";
    static final String ARTIFACT = "feign-jackson";
    static final Set<String> SOURCE_VERSIONS = Set.of("10.4.0", "11.1", "12", "12.4");
    ...
    private static final Set<String> GENERATED_DIRECTORIES = Set.of(
            ...
            ".next", ".cache", "coverage", ".output", "storybook-static", "reports", "test-results");

    @Override
    public String getDisplayName() {
        return "Upgrade workbook-selected Feign Jackson dependencies to 13.6";
    }

    @Override
    public String getDescription() {
        return "Upgrade only exact io.github.openfeign:feign-jackson versions listed by the workbook in owned " +
               "Maven or root Gradle declarations without guessing external version owners or variants.";
    }
    ...
    private static Xml.Document migratePom(Xml.Document document, ExecutionContext ctx) {
        ...
        return (Xml.Document) new XmlIsoVisitor<ExecutionContext>() {
            ...
            public Xml.Tag visitTag(Xml.Tag tag, ExecutionContext ec) {
                ...
                if (isFeignJacksonDependency(getCursor(), t) && t.getChildValue("version").map(String::trim)
    ...
    static boolean isFeignJacksonDependency(Cursor cursor, Xml.Tag tag) {
    ...
    private static boolean targetVersionReference(Cursor cursor, String text) {
        ...
        return dependencyCursor.getValue() instanceof Xml.Tag dependency &&
               isFeignJacksonDependency(dependencyCursor, dependency);
    }
