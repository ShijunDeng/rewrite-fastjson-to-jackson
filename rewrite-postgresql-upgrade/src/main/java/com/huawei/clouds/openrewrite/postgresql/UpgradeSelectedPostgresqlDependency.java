...
/** Upgrades only the exact PostgreSQL JDBC source versions selected by the migration workbook. */
public final class UpgradeSelectedPostgresqlDependency extends Recipe {
    static final String GROUP = "org.postgresql";
    static final String ARTIFACT = "postgresql";
    static final Set<String> SOURCE_VERSIONS = Set.of("1.17.6", "14.8", "42.2.19", "42.2.5", "42.5.1", "42.5.4", "42.5.5", "42.5.6", "42.6.0", "42.6.1");
    static final String TARGET = "42.7.13";
    ...
    private static final Set<String> EXCLUDED_DIRECTORIES = Set.of(
            "target", "build", "out", "dist", ".gradle", ".mvn", ".m2", ".idea", ".angular", ".nx",
            ".next", ".cache", ".output", "node_modules", "bower_components", "vendor", ".pnpm", ".yarn",
            ".npm", ".git", ".vscode", ".nuxt", ".turbo", ".parcel-cache", ".vite",
            "coverage", "tmp", "temp", "report", "reports", "test-results", "storybook-static");

    @Override
    public String getDisplayName() {
        return "Upgrade workbook-selected PostgreSQL JDBC dependencies to 42.7.13";
    }

    @Override
    public String getDescription() {
        return "Upgrade only exact org.postgresql:postgresql source versions listed by the workbook in owned " +
               "Maven or root Gradle dependency declarations, without guessing ranges or external version owners.";
    }
    ...
    private static Xml.Document migratePom(Xml.Document document, ExecutionContext ctx) {
        ...
        return (Xml.Document) new XmlIsoVisitor<ExecutionContext>() {
            ...
            public Xml.Tag visitTag(Xml.Tag tag, ExecutionContext ec) {
                ...
                if (isTargetDependency(getCursor(), t) && t.getChildValue("version").map(String::trim)
    ...
    private static boolean targetVersionReference(Cursor cursor, String text) {
        ...
        return dependencyCursor.getValue() instanceof Xml.Tag dependency &&
               isTargetDependency(dependencyCursor, dependency);
    }
