...
/** Updates only npm single-version declarations visible in the supplied migration spreadsheet. */
public final class UpgradeSelectedAngularRouterDependency extends Recipe {
    ...
    public String getDisplayName() {
        return "Upgrade selected @angular/router declarations to 20.3.26";
    }

    @Override
    public String getDescription() {
        return "Updates exact, caret, or tilde single-version @angular/router declarations selected by the " +
               "spreadsheet without changing complex ranges, aliases, protocols, variables, metadata, or lockfiles.";
    }
    ...
    public TreeVisitor<?, ExecutionContext> getVisitor() {
        return new JsonIsoVisitor<ExecutionContext>() {
            ...
            public Json.Member visitMember(Json.Member member, ExecutionContext ctx) {
                ...
                if (!isPackageJson() || !"@angular/router".equals(key(visited)) ||
    ...
    static boolean selected(String declaration) {
        ...
        return SOURCE_VERSIONS.contains(candidate) &&
               declaration.matches("[~^]?" + java.util.regex.Pattern.quote(candidate));
    }
