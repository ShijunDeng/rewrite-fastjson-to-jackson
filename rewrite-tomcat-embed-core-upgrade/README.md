...
| `com.huawei.clouds.openrewrite.tomcatembedcore.MigrateTomcatEmbedCore101Java` | AUTO | Composes the return-type-safe official Servlet 6 leaves plus the official EL spelling and RFC 6265 Cookie recipes; the unsafe `getValueNames()` rename is excluded and marked. |
...
| `com.huawei.clouds.openrewrite.tomcatembedcore.MigrateTomcatEmbedCoreTo10_1_57` | RECOMMENDED | Captures the exact pre-upgrade version at the nearest build root, guards branch transitions, applies only project-eligible AUTO changes, then reports only that project's remaining risks. |
...
The recommended entry adds a second, project-level safety boundary. Before the dependency edit runs, a scanning recipe records the nearest Maven or Gradle build root and its exact source version in a non-printing marker. Source, configuration, resource and project-specific risk recipes can run only below a root that owns exactly one non-conflicting approved source version. A nested Maven/Gradle root always forms a new boundary, so an unrelated child cannot inherit eligibility from an approved parent. Javax→Jakarta dependency/package changes additionally require the recorded source to be Tomcat 9; an approved Tomcat 10.1 source does not replay the 9→10 namespace transition.

Consequently, an initial `10.1.57`, a higher version, an out-of-list version such as `10.0.27`, an unrelated coordinate, or a source file without a qualifying local build root is a complete recommended-entry NOOP. The only deliberate exception is the two supplied Tomcat 11 conflicts: their build declarations receive `目标版本冲突（禁止降级）` semantics through the conflict marker while dependency, Java and configuration content remain unchanged.
...
- `HttpSession.getValueNames()` is not automatically renamed. The official aggregate performs a method-name-only edit, but `getValueNames()` returns `String[]` while `getAttributeNames()` returns `Enumeration<String>`; the module emits a dedicated marker requiring explicit iteration, collection and public-return-type adaptation.
...
| --- | --- | --- |
| OpenRewrite core coordinate `8.87.5` | Resolved artifact manifest commit [`91e23c2858176877428ddc03e146d2bb023217a8`](https://github.com/openrewrite/rewrite/tree/91e23c2858176877428ddc03e146d2bb023217a8) (manifest implementation `8.88.0-SNAPSHOT`); `rewrite-java` SHA-256 `a378253fe0c0865ab39d1743e468fe3d2557d7760e0a6897de294ca18ea90043`; `rewrite-maven` SHA-256 `d7d4a38376a87e9de2a27f43dfe522abba6a8e20ca3429587ec349fcde23db4c` | Reuse official [`ChangePackage`](https://github.com/openrewrite/rewrite/blob/91e23c2858176877428ddc03e146d2bb023217a8/rewrite-java/src/main/java/org/openrewrite/java/ChangePackage.java), `ChangeMethodName`, `DeleteMethodArgument`, `ReorderMethodArguments` and [`DoesNotUseType`](https://github.com/openrewrite/rewrite/blob/91e23c2858176877428ddc03e146d2bb023217a8/rewrite-java/src/main/java/org/openrewrite/java/search/DoesNotUseType.java). Local code is retained only for exact-version/build-root ownership, Tomcat-specific XML and risk searches. |
...
| `rewrite-migrate-java` `3.40.0` | [`658481254a6ee678f5f162e51d8d49ee01c75877`](https://github.com/openrewrite/rewrite-migrate-java/tree/658481254a6ee678f5f162e51d8d49ee01c75877); SHA-256 `8c00217ff2cf4dc9c139a1eff49ed1403fe20e010e42295f5aeb1dd9a5872dc6` | Reuse the safe leaf recipes from `RemovalsServletJakarta10`, and reuse `UpdateGetRealPath`, `RemovedIsParmetersProvidedMethod` and `ServletCookieBehaviorChangeRFC6265` directly. The official aggregate itself is intentionally not activated because its `getValueNames()` name-only rewrite ignores the return-type change. |
...
The module currently executes 437 tests with zero failures. They include every approved source literal in direct Maven, exclusive-property Maven, Gradle Groovy and Gradle Kotlin forms; all 38 approved versions through the recommended project gate; target/higher/out-of-list/unrelated complete-NOOP cases; nearest nested-root isolation; both blocked Tomcat 11 values; exact Servlet/EL dependency migrations and newer Jakarta API no-downgrade cases; official composition-tree guards; real-repository fixtures; positive, negative, lookalike, owner, overload, generated-path, marker-survival, aggregate-order and two-cycle idempotence cases.
...
Tests follow OpenRewrite's pinned [`RewriteTest`/cycle assertions](https://github.com/openrewrite/rewrite/blob/91e23c2858176877428ddc03e146d2bb023217a8/rewrite-test/src/main/java/org/openrewrite/test/RewriteTest.java), [`ChangePackageTest`](https://github.com/openrewrite/rewrite/blob/91e23c2858176877428ddc03e146d2bb023217a8/rewrite-java-test/src/test/java/org/openrewrite/java/ChangePackageTest.java), and [`SearchResult` marker](https://github.com/openrewrite/rewrite/blob/91e23c2858176877428ddc03e146d2bb023217a8/rewrite-java/src/main/java/org/openrewrite/java/search/UsesType.java) patterns. Each fixture test identifies any namespace-only reduction in its comment.
