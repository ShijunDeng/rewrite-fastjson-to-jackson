...
public final class UpgradeSelectedJschDependency extends Recipe {
    static final String GROUP = "com.github.mwiede";
    static final String ARTIFACT = "jsch";
    static final Set<String> SOURCE_VERSIONS = Set.of("0.1.55", "0.1.70", "0.2.3", "0.2.7", "0.2.9");
    static final String TARGET = "2.27.7";
    ...
    public String getDisplayName() {
        return "Upgrade workbook-selected mwiede JSch dependencies to 2.27.7";
    }

    @Override
    public String getDescription() {
        return "Upgrade only exact com.github.mwiede:jsch source versions listed by the workbook in owned Maven " +
    ...
    private static Xml.Document migratePom(Xml.Document document, ExecutionContext ctx) {
        ...
        Map<PropertyOwner, Integer> jschReferences = new HashMap<>();
        new XmlIsoVisitor<ExecutionContext>() {
            @Override
            public Xml.CharData visitCharData(Xml.CharData charData, ExecutionContext executionContext) {
                ...
                collectReferences(visited.getText(), getCursor(), definitions, allReferences,
                        jschVersionReference(getCursor(), visited.getText()) ? jschReferences : null);
                return visited;
            }
        ...
        Set<PropertyOwner> safeProperties = jschReferences.keySet().stream()
                ...
                .filter(owner -> jschReferences.getOrDefault(owner, 0) > 0)
                .filter(owner -> allReferences.getOrDefault(owner, 0)
                        .equals(jschReferences.getOrDefault(owner, 0)))
        ...
        return (Xml.Document) new XmlIsoVisitor<ExecutionContext>() {
            ...
            public Xml.Tag visitTag(Xml.Tag tag, ExecutionContext executionContext) {
                ...
                if (isJschDependency(getCursor(), t) && t.getChildValue("version").map(String::trim)
    ...
    static boolean isJschDependency(Cursor cursor, Xml.Tag tag) {
    ...
    private static boolean jschVersionReference(Cursor cursor, String text) {
        ...
        return dependencyCursor.getValue() instanceof Xml.Tag dependency &&
               isJschDependency(dependencyCursor, dependency);
    }
