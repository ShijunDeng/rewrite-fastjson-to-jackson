...
public final class UpgradeSelectedJaxbImplDependency extends Recipe {
    private static final String PREFIX = JaxbImplSupport.GROUP + ":" + JaxbImplSupport.ARTIFACT + ":";
    private static final Pattern PROPERTY_REFERENCE = Pattern.compile("\\$\\{([^}]+)}");

    @Override
    public String getDisplayName() {
        return "Upgrade workbook-selected JAXB implementation dependencies to 4.0.6";
    }

    @Override
    public String getDescription() {
        return "Upgrade only exact com.sun.xml.bind:jaxb-impl versions 2.3.8, 4.0.2, and 4.0.3 in owned " +
                ...
                if (tree instanceof G.CompilationUnit groovy && fileName.endsWith(".gradle")) {
                    return new GroovyIsoVisitor<ExecutionContext>() {
                        ...
                        public J.MethodInvocation visitMethodInvocation(J.MethodInvocation method,
                            ...
                            if (coordinate(JaxbImplSupport.mapValue(m, "group"), JaxbImplSupport.mapValue(m, "name")) &&
                                JaxbImplSupport.SOURCES.contains(JaxbImplSupport.mapValue(m, "version")) &&
                                !JaxbImplSupport.hasVariant(m)) {
                                return m.withArguments(m.getArguments().stream().map(argument ->
                                        argument instanceof G.MapEntry entry && "version".equals(JaxbImplSupport.mapKey(entry)) &&
                                        entry.getValue() instanceof J.Literal literal
                                                ? entry.withValue(JaxbImplSupport.replaceLiteral(literal, JaxbImplSupport.TARGET))
                                                : argument).toList());
                            }
                            return m.withArguments(m.getArguments().stream().map(argument ->
    ...
    private static Xml.Document migratePom(Xml.Document document, ExecutionContext ctx) {
        ...
        new XmlIsoVisitor<ExecutionContext>() {
            ...
            public Xml.CharData visitCharData(Xml.CharData charData, ExecutionContext executionContext) {
                ...
                collectReferences(visited.getText(), getCursor(), definitions, allReferences,
                        jaxbVersionReference(getCursor(), visited.getText()) ? targetReferences : null);
                return visited;
            }
        ...
        Set<PropertyOwner> safeProperties = targetReferences.keySet().stream()
                .filter(owner -> values.containsKey(owner) && JaxbImplSupport.SOURCES.contains(values.get(owner)))
        ...
        return (Xml.Document) new XmlIsoVisitor<ExecutionContext>() {
            ...
            public Xml.Tag visitTag(Xml.Tag tag, ExecutionContext executionContext) {
                ...
                if (JaxbImplSupport.isMavenPropertyDefinition(getCursor(), t) &&
                    ...
                    t.getValue().map(String::trim).filter(JaxbImplSupport.SOURCES::contains).isPresent()) {
                    return t.withValue(JaxbImplSupport.TARGET);
                }
                if (JaxbImplSupport.isJaxbImplDependency(getCursor(), t) && JaxbImplSupport.standardJar(t) &&
                    t.getChildValue("version").map(String::trim).filter(JaxbImplSupport.SOURCES::contains).isPresent()) {
                    return t.withChildValue("version", JaxbImplSupport.TARGET);
                }
                return t;
            }
        }.visitNonNull(document, ctx);
    ...
    private static boolean jaxbVersionReference(Cursor cursor, String text) {
        ...
        return dependencyCursor.getValue() instanceof Xml.Tag dependency &&
               JaxbImplSupport.isJaxbImplDependency(dependencyCursor, dependency) &&
               JaxbImplSupport.standardJar(dependency);
    }
    ...
    private static boolean coordinate(String group, String artifact) {
        return JaxbImplSupport.GROUP.equals(group) && JaxbImplSupport.ARTIFACT.equals(artifact);
    }

    private static G.MapLiteral upgradeMap(G.MapLiteral map) {
        if (!coordinate(JaxbImplSupport.mapValue(map, "group"), JaxbImplSupport.mapValue(map, "name")) ||
            !JaxbImplSupport.SOURCES.contains(JaxbImplSupport.mapValue(map, "version")) ||
            JaxbImplSupport.hasVariant(map)) return map;
        return map.withElements(map.getElements().stream().map(entry ->
                "version".equals(JaxbImplSupport.mapKey(entry)) && entry.getValue() instanceof J.Literal literal
                        ? entry.withValue(JaxbImplSupport.replaceLiteral(literal, JaxbImplSupport.TARGET)) : entry).toList());
    }
    ...
    private static J.Literal upgradeCoordinate(J.Literal literal) {
        if (!(literal.getValue() instanceof String value) || !value.startsWith(PREFIX) ||
            !JaxbImplSupport.SOURCES.contains(value.substring(PREFIX.length()))) return literal;
        return JaxbImplSupport.replaceLiteral(literal, PREFIX + JaxbImplSupport.TARGET);
    }
}
