...
/** Upgrade only workbook-selected versions in declarations owned by the current build file. */
public final class UpgradeSelectedJUnitJupiterApiDependency extends Recipe {
    static final String GROUP = "org.junit.jupiter";
    static final String ARTIFACT = "junit-jupiter-api";
    static final Set<String> SOURCE_VERSIONS = Set.of("5.7.1", "5.8.2", "5.9.3");
    static final String TARGET = "6.0.1";
    ...
    public String getDisplayName() {
        return "Upgrade workbook-selected JUnit Jupiter API dependencies to 6.0.1";
    }

    @Override
    public String getDescription() {
        return "Upgrade only exact org.junit.jupiter:junit-jupiter-api versions 5.7.1, 5.8.2, and 5.9.3 in " +
               "owned Maven or root Gradle declarations; external owners, variants, and ranges remain untouched.";
    }
    ...
    private static Xml.Document migratePom(Xml.Document document, ExecutionContext ctx) {
        ...
        Set<PropertyOwner> safeProperties = ownedReferences.keySet().stream()
                ...
                .filter(owner -> allReferences.getOrDefault(owner, 0).equals(ownedReferences.getOrDefault(owner, 0)))
        ...
        return (Xml.Document) new XmlIsoVisitor<ExecutionContext>() {
            ...
            public Xml.Tag visitTag(Xml.Tag tag, ExecutionContext ec) {
                ...
                if (isJUnitJupiterApiDependency(getCursor(), t) && t.getChildValue("version").map(String::trim)
    ...
    static boolean isJUnitJupiterApiDependency(Cursor cursor, Xml.Tag tag) {
    ...
    private static boolean targetVersionReference(Cursor cursor, String text) {
        ...
        return dependencyCursor.getValue() instanceof Xml.Tag dependency &&
               isJUnitJupiterApiDependency(dependencyCursor, dependency);
    }
