...
final class SpringRetrySupport {
    static final String GROUP = "org.springframework.retry";
    static final String ARTIFACT = "spring-retry";
    static final String SOURCE = "1.3.4";
    static final String TARGET = "2.0.13";
    ...
    static boolean fixedVersion(String version) {
        return version != null && FIXED_NUMERIC.matcher(version.trim()).matches();
    }

    static boolean targetConflict(String version) {
        if (version == null) return false;
        Matcher matcher = FIXED_NUMERIC.matcher(version.trim());
        ...
        int majorOrder = major.compareTo(BigInteger.valueOf(2));
        int minorOrder = minor.compareTo(BigInteger.ZERO);
        return majorOrder > 0 ||
               majorOrder == 0 && minorOrder > 0 ||
               majorOrder == 0 && minorOrder == 0 && patch.compareTo(BigInteger.valueOf(13)) > 0;
    }

    static boolean isSpringRetryDependency(Cursor cursor, Xml.Tag tag) {
