...
class UpgradeNettyCodecHttp2DependencyTest implements RewriteTest {
    private static final String[] APPROVED = {"4.1.100.Final", "4.1.101.Final", "4.1.108.Final",
            "4.1.109.Final", "4.1.112.Final", "4.1.118.Final", "4.1.124.Final", "4.1.125.Final",
            "4.1.126.Final", "4.1.128.Final", "4.1.129.Final", "4.1.130.Final", "4.1.132.Final"};

    @Override
    public void defaults(RecipeSpec spec) {
        spec.recipe(new UpgradeSelectedNettyCodecHttp2Dependency());
    }

    @ParameterizedTest(name = "approved 4.1 source {0}")
    @ValueSource(strings = {"4.1.100.Final", "4.1.101.Final", "4.1.108.Final", "4.1.109.Final",
            "4.1.112.Final", "4.1.118.Final", "4.1.124.Final", "4.1.125.Final", "4.1.126.Final",
            "4.1.128.Final", "4.1.129.Final", "4.1.130.Final", "4.1.132.Final"})
    void upgradesEveryApproved41Source(String version) {
        rewriteRun(xml(pom(version), pom(NettyCodecHttp2Support.TARGET), source -> source.path("pom.xml")));
    }

    @Test
    void whitelistSeparatesTargetConflictsFromAutoSources() {
        assertEquals(Set.of(APPROVED), NettyCodecHttp2Support.AUTO_SOURCES);
        assertEquals(Set.of("4.2.10.Final", "4.2.12.Final"), NettyCodecHttp2Support.TARGET_CONFLICTS);
        assertEquals(15, NettyCodecHttp2Support.WORKBOOK_SOURCES.size());
    }
    ...
    void profileOverrideWinsWithoutLeakingToRoot() {
        rewriteRun(xml(
                project("<properties><v>4.2.12.Final</v></properties><profiles><profile><id>it</id>" +
    ...
    void upgradesGroovyStringMapAndMapLiteralNotation() {
        rewriteRun(
                buildGradle("dependencies { implementation 'io.netty:netty-codec-http2:4.1.100.Final' }",
                        "dependencies { implementation 'io.netty:netty-codec-http2:4.1.136.Final' }"),
                buildGradle("dependencies { runtimeOnly group: 'io.netty', name: 'netty-codec-http2', version: '4.1.128.Final', transitive: false }",
                        "dependencies { runtimeOnly group: 'io.netty', name: 'netty-codec-http2', version: '4.1.136.Final', transitive: false }"),
                buildGradle("dependencies { implementation([group: 'io.netty', name: 'netty-codec-http2', version: '4.1.132.Final']) }",
                        "dependencies { implementation([group: 'io.netty', name: 'netty-codec-http2', version: '4.1.136.Final']) }"));
    }
    ...
    void upgradesEveryApprovedSourceInGroovy(String version) {
        rewriteRun(buildGradle("dependencies { implementation 'io.netty:netty-codec-http2:" + version + "' }",
                "dependencies { implementation 'io.netty:netty-codec-http2:4.1.136.Final' }"));
    }
    ...
    void upgradesKotlinGradleNotation() {
        rewriteRun(buildGradleKts(
                "dependencies { implementation(\"io.netty:netty-codec-http2:4.1.130.Final\") }",
                "dependencies { implementation(\"io.netty:netty-codec-http2:4.1.136.Final\") }"));
    }
    ...
    static Stream<Arguments> nestedGradle() {
        String d = "'io.netty:netty-codec-http2:4.1.100.Final'";
        return Stream.of(
    ...
    @ValueSource(strings = {"4.1.99.Final", "4.1.102.Final", "4.1.107.Final", "4.1.110.Final",
            "4.1.111.Final", "4.1.113.Final", "4.1.117.Final", "4.1.119.Final", "4.1.123.Final",
            "4.1.127.Final", "4.1.131.Final", "4.1.133.Final", "4.1.135.Final", "4.1.137.Final",
    ...
    void versionlessBomOwnedVariantsAndPluginDependenciesAreNoop() {
        rewriteRun(
                ...
                buildGradle("dependencies { implementation 'io.netty:netty-codec-http2:4.1.129.Final:tests' }"),
                buildGradle("dependencies { implementation 'io.netty:netty-codec-http2:4.1.130.Final@zip' }"));
    }
    ...
    void twoCyclesAreIdempotent() {
        rewriteRun(spec -> spec.cycles(2).expectedCyclesThatMakeChanges(1),
                ...
                buildGradle("dependencies { implementation 'io.netty:netty-codec-http2:4.1.132.Final' }",
                        "dependencies { implementation 'io.netty:netty-codec-http2:4.1.136.Final' }"));
    }
    ...
    void publicStrictRecipeContainsOnlySelectedUpgrade() {
        Environment environment = Environment.builder()
                .scanRuntimeClasspath("com.huawei.clouds.openrewrite.nettycodechttp2").build();
        var recipe = environment.activateRecipes(
                "com.huawei.clouds.openrewrite.nettycodechttp2.UpgradeNettyCodecHttp2To4_1_136");
        assertEquals("com.huawei.clouds.openrewrite.nettycodechttp2.UpgradeSelectedNettyCodecHttp2Dependency",
    ...
    static String dep(String version, String extra) {
        return "<dependency><groupId>io.netty</groupId><artifactId>netty-codec-http2</artifactId>" +
               "<version>" + version + "</version>" + extra + "</dependency>";
    }

    static String depWithoutVersion() {
        return "<dependency><groupId>io.netty</groupId><artifactId>netty-codec-http2</artifactId></dependency>";
    }
}
